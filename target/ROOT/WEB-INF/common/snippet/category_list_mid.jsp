...
<div class="s-icon s-all">
	<c:choose>
		<c:when test="${orignKeyword != '*' && categoryLayer > 0 && categoryList.link.url != ''}"><a href="${basePath}/s/Search?key=${keyword}" data-attr='1003' style='color:#000'>所有分类</a></c:when>
...
<dl class="m-dl">
	...
	<c:choose>
		<c:when test="${layer == 0 || layer == 1}">
			<c:forEach items="${categoryList.linkTree}" var="category"><c:forEach items="${category.linkTree}" var="categoryLayer1"><dd><a class="s-icon" data-attr='1003' href="${categoryLayer1.link.url}">${categoryLayer1.link.text}</a></dd></c:forEach></c:forEach>
		</c:when>
		<c:when test="${layer == 2}">
			<c:forEach items="${categoryList.linkTree}" var="category" >
				...
				<c:forEach items="${category.linkTree}" var="categoryLayer1">
					<c:if test="${categoryLayer1.link.clicked && fn:length(categoryLayer1.linkTree) < 1}">
					   <c:forEach items="${category.linkTree}" var="categoryLayer1">
					       <dd><a class="s-icon ${categoryLayer1.link.clicked ? 'cur':''}" data-attr='1003' href="${categoryLayer1.link.url}">${categoryLayer1.link.text}</a></dd>
					   </c:forEach>
					</c:if>
					<c:if test="${categoryLayer1.link.clicked && fn:length(categoryLayer1.linkTree) >= 1}">
