...
<%@ page session="false"%>
<c:set value="false" var="filterDisplay"/>
<div class="row" id="filter-attr">
    <div class="container">
      <div class="grid_10">
		<div class="filter-area margin-fix">
			<c:if test="${fn:length(filterList)>0}">                            
				<c:set value="true" var="filterDisplay"/>
				<div class="filter-on">
					<dl>
				...
				<c:forEach items="${attrsLinks}" var="attrLinks" varStatus="stat">
					...
					<dl class="filter-item" style="${hide ? 'display:none':''}"><c:set value="${hiddenAttrNum + 1}" var="hiddenAttrNum"/>
						...
						<dd class="filter-lists cf">
							<c:forEach items="${attrLinks.attrList}" var="attr"><a href="${attr.url}" data-attr="1002" title="${attr.text}">${attr.text}</a></c:forEach>
						</dd>
						<div class="filter-act">
							<c:if test="${fn:length(attrLinks.attrList)>2}">
								<a href="javascript:void(0);" class="btn-multiple"><i class="icon-search-add">多选</i>&nbsp;多选</a>
							</c:if>
			...
			<div class="filter-more">
				<c:if test="${hiddenAttr != null && hiddenAttr != ''}">
					<a href="?ls=true" id="J_more" class="open">
						<span class="other-attr">更多<%-- <em>（${hiddenAttr} ${hiddenAttrNum > 9 ? '等' : ''}）</em> --%></span>
...
</div><script type="text/javascript">if('${filterDisplay}' == 'false'){document.getElementById("filter-attr").style.display = "none";}</script>
